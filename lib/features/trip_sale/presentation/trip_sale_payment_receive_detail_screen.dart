...
class TripSalePaymentReceiveDetailScreen extends ConsumerWidget {
  const TripSalePaymentReceiveDetailScreen({super.key, required this.tripSaleReceipt});
  final TripSaleReceipt tripSaleReceipt;
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // final hasPRModuleAccess = ref.watch(permissionProvider.notifier).hasAccessToModule(SUBMODULE.paymentReceive, MODULE.trip);
    ...
    return Scaffold(
        ...
        actions: [
          CustomPopUpMenuButton(
            popupMenuEntries: [
              ...
              //         onPressed: (v) {
              //           ref.read(asyncTripSaleFormNotifierProvider.notifier).deletePayment(tripSaleReceipt.id, v);
              //         }),
              //     isLast: true,
              ...
              CustomPopUpMenuItem(
                ...
                ontap: () async {
                  await PrintServiceV2.excutePayment(
                    context,
                    PaymentPrintFormat(
                      invoiceID: tripSaleReceipt.invoiceCode,
                      receiveDate: tripSaleReceipt.paymentReciveDate,
                      receiveID: tripSaleReceipt.code,
                      paymentAmount: tripSaleReceipt.paymentReceiveAmount,
                      paymentMethod: tripSaleReceipt.paymentMethod.name,
                    ),
                  );
                },
      ...
      body: AsyncValueWidget(
        errorCallback: () => ref.invalidate(paymentReceiveByIdProvider(tripSaleReceipt.id)),
        value: ref.watch(paymentReceiveByIdProvider(tripSaleReceipt.id)),
        data: (data) => Padding(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        InfoCard(title: "Payment Receive ID", value: tripSaleReceipt.code),
                        InfoCard(title: "Payment Receive Date", value: prettierDateFormat(tripSaleReceipt.paymentReciveDate)),
                        InfoCard(title: "Invoice ID", value: tripSaleReceipt.invoiceCode),
                        InfoCard(title: "Payment Method", value: tripSaleReceipt.paymentMethod.name),
                        InfoCard(title: "Payment Receive Amount", value: formatter.format(tripSaleReceipt.paymentReceiveAmount)),
