...
class SaleTypeCard extends ConsumerWidget {
  const SaleTypeCard({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final paymentTpye = ref.watch(allFiltersProvider.select((value) => value.saleType));
    return FilterCard(
        child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const HeaderTextLarge("Sale Type"),
        const SizedBox(height: 24),
        AsyncValueWidget(
          value: ref.watch(saleTypesProvider),
          data: (data) {
            return Column(
              children: data
                  .map((e) => InkWell(
                        onTap: () => ref.read(allFiltersProvider.notifier).setSaleType(e),
                        child: Padding(
                                ...
                                child: Transform.scale(
                                  ...
                                  child: Checkbox(
                                    ...
                                    onChanged: (bool? value) {
                                      ref.read(allFiltersProvider.notifier).setSaleType(e);
                                    },
                                  ),
                                ),
                              ),
