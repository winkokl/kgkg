...
class MakeConsignmentPaymentReceiveScreen extends HookConsumerWidget {
  const MakeConsignmentPaymentReceiveScreen({
    super.key,
    required this.saleInvoice,
  });
  final ConsignmentInvoice saleInvoice;
    ...
    ref.listenX(
      ...
      onData: (res) => CustomDialog.showDoneAndPrintDialog(
        ...
        printOnPressed: () async {
          final consignmentReceipt = res.data as ConsignmentReceipt;
          LoadingOverlay.show();
              ...
              child: ActionButton(
                  ...
                  onPressed: () {
                    if (formKey.currentState!.validate() == true) {
                      ...
                      ref.read(asyncConsignmentFormNotifierProvider.notifier).makePaymentReceive(
                          attachment.value,
                          signature.value!,
                          ConsignmentReceipt(
                            paymentReciveDate: paymentReceiveDate.value,
                            paymentReceiveAmount: paidAmountController.text.toDouble(),
                            consignmentInvoiceId: saleInvoice.id,
