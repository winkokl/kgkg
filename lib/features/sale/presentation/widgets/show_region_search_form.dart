...
Future<List<Region>?> showRegionSearchForm(
                  ...
                  children: [
                    Padding(
                      ...
                      child: Row(
                        children: [
                          const Spacer(),
                          const HeaderText("Select Regions", color: brandColor),
                          const Spacer(),
                          InkWell(
                    ...
                    const Padding(
                      ...
                      child: SearchTextField(title: 'rn'),
                    ),
                    const SizedBox(height: 16),
                    Expanded(
                      child: Padding(
                        ...
                        child: CommonPagingView(
                          provider: paginatedRegionNotifierProvider,
                          contentBuilder: (data, endItem) => ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return InkWell(
                                onTap: () {
                                  if (regs.value.map((e) => e.id).contains(data.items[index].id)) {
                                    regs.value = [
                                      for (final v in regs.value)
                                        if (v.id != data.items[index].id) v
                                    ];
                                  } else {
                                    regs.value = [...regs.value, data.items[index]];
                                  }
                                },
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Row(
                                      children: [
                                        Expanded(child: HeaderText(data.items[index].name)),
                                        SizedBox(
                                          width: 20,
                                          height: 20,
                                          child: Transform.scale(
                                            ...
                                            child: Checkbox(
                                              ...
                                              value: regs.value.contains(data.items[index]),
                    ...
                    Padding(
                      ...
                      child: ActionButton(
                        label: "Done",
                        onPressed: () => Navigator.of(context).pop(regs.value),
                      ),
                    )
                  ],
                ),
