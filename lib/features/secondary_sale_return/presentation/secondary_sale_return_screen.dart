...
import 'package:mgkaung_dms_mobile/features/secondary_sale_return/presentation/widgets/make_payment_tab.dart';
import 'package:mgkaung_dms_mobile/features/secondary_sale_return/presentation/widgets/secondary_sale_return_tab.dart';
import 'package:flutter/material.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';

class SecondarySaleReturnScreen extends HookConsumerWidget {
  const SecondarySaleReturnScreen({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final hasSsReturnModuleAccess = ref.watch(permissionProvider.notifier).hasAccessToModule(SUBMODULE.salesReturn, MODULE.secondarySalesReturn);
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: false,
        actions: !hasSsReturnModuleAccess.contains(PERMISSION.create)
            ? null
            : [
                AddButton(
                  onTap: () {
                    ...
                    ref.watch(goRouterProvider).push(const SecondarySaleReturnFormRoute(isEdit: false).location);
                  },
                )
              ],
      ),
