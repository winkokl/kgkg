...
class SaleReturnTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return CommonPagingView(
      provider: paginatedConsignmentReturnNotifierProvider,
      contentBuilder: (data, endItem) => ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final consignmentReturn = data.items[index];
          final status = consignmentReturn.paymentStatus;
          return CustomCard(
            title: consignmentReturn.customer.name,
            subtitle: consignmentReturn.code,
            extraSubtitle: consignmentReturn.consignmentContract.code,
            ...
            date: prettierDateFormat(consignmentReturn.returnDate),
            amount: consignmentReturn.totalReturnAmount,
            ontap: () => ref.watch(goRouterProvider).push(const ConsignmentReturnDetailRoute().location, extra: consignmentReturn),
          );
        },
      ),
    );
