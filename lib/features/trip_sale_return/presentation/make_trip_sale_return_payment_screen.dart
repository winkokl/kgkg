...
class MakeTripSaleReturnPaymentScreen extends HookConsumerWidget {
  const MakeTripSaleReturnPaymentScreen({
    super.key,
    required this.tripSaleReturnReceipt,
  });
  final TripSaleReturnReceipt tripSaleReturnReceipt;
    ...
    ref.listenX(
      ...
      onData: (res) => CustomDialog.showDoneAndPrintDialog(
        ...
        printOnPressed: () async {
          final tripReceipt = res.data as TripSaleReturnReceipt;
          LoadingOverlay.show();
          await PrintServiceV2.excutePayment(
              context,
              PaymentPrintFormat(
                invoiceID: tripReceipt.invoiceCode,
                receiveDate: prettierDateFormat(tripReceipt.paymentDate),
                receiveID: tripReceipt.code,
                paymentAmount: tripReceipt.paidAmount,
                paymentMethod: tripReceipt.paymentMethod.name,
              ));
          LoadingOverlay.hide();
        },
      ),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          FormTextInput(
                            ...
                            initialValue: tripSaleReturnReceipt.code,
                          ),
                          const SizedBox(height: 20),
                          FormTextInput(
                            label: "Return Date",
                            initialValue: prettierDateFormat(tripSaleReturnReceipt.returnDate),
                            isReadOnly: true,
                          ...
                          FormTextInput(
                            ...
                            validator: FormValidators.requiredValidator.call(),
                            ontap: () async {
                              final selectedDate = await CustomDatePicker.datePicker(context, "", tripSaleReturnReceipt.returnDate);
                          ...
                          FormTextInput(
                            ...
                            initialValue: formatter.format(tripSaleReturnReceipt.totalReturnAmount),
                            isReadOnly: true,
                            fillColor: textFieldFillColor,
                            textStyle: readOnlyTextStyle,
                          ),
                          ...
                          FormTextInput(
                            label: "Balance",
                            initialValue: formatter.format(tripSaleReturnReceipt.balance),
                            isReadOnly: true,
                            fillColor: textFieldFillColor,
                            textStyle: readOnlyTextStyle,
                          ),
                          ...
                          FormTextInput(
                            ...
                            validator: (v) {
                              ...
                              } else if (v.toDouble() > tripSaleReturnReceipt.balance) {
              ...
              child: ActionButton(
                  ...
                  onPressed: () {
                    if (formKey.currentState!.validate() == true) {
                      ...
                      final newReceipt = tripSaleReturnReceipt.copyWith(
                      ...
                      ref.read(asyncTripSaleReturnFormNotifierProvider.notifier).makePaymentReceive(attachment.value, signature.value!, newReceipt);
                    }
                  }),
            ),
          )
