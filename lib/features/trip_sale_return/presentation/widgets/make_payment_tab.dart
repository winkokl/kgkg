...
class MakePaymentTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return CommonPagingView(
      provider: paginatedTripSaleReturnReceiptNotifierProvider,
      contentBuilder: (data, endItem) => ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final tripSaleReturnReceipt = data.items[index];

          return ListTileCard(
            ontap: () => ref.watch(goRouterProvider).push(const TripSaleReturnPaymentDetailRoute().location, extra: tripSaleReturnReceipt),
            titleWidget: HeaderText(tripSaleReturnReceipt.code),
            subtitleWidgets: [
              const SizedBox(height: 6),
              HeaderTextSmall(prettierDateFormat(tripSaleReturnReceipt.paymentDate), color: brandColor),
            ],
            trailingWidget: Column(
              crossAxisAlignment: CrossAxisAlignment.end,
              mainAxisAlignment: MainAxisAlignment.center,
              children: [HeaderText(formatter.format(tripSaleReturnReceipt.paidAmount)), const HeaderTextSmall("MMK", color: secondaryTextColor)],
            ),
          );
        },
      ),
