...
class SaleReturnTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return CommonPagingView(
      provider: paginatedTripSaleReturnNotifierProvider,
      contentBuilder: (data, endItem) => ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final tripSaleReturn = data.items[index];
          final status = tripSaleReturn.paymentStatus;
          return CustomCard(
            title: tripSaleReturn.customer.name.isEmpty ? '-' : tripSaleReturn.customer.name,
            subtitle: tripSaleReturn.code,
            extraSubtitle: tripSaleReturn.returnType == 1 ? tripSaleReturn.invoiceCode : tripSaleReturn.tripSaleRequest.tripSaleRequestCode,
            ...
            date: prettierDateFormat(tripSaleReturn.returnDate),
            amount: tripSaleReturn.totalReturnAmount,
            ontap: () => ref.watch(goRouterProvider).push(const TripSaleReturnDetailRoute().location, extra: tripSaleReturn),
          );
        },
      ),
    );
