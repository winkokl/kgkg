...
Future<void> showTripSaleInvoiceSearchForm(BuildContext context, Function(TripSaleInvoice saleInvoice) onSelect) async {
                          ...
                          Expanded(
                            child: Padding(
                              ...
                              child: CommonPagingView(
                                provider: paginatedTripSaleInvoiceNotifierProvider,
                                contentBuilder: (data, endItem) => ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    return InkWell(
                                      onTap: () async {
                                        final future = ref.watch(tripSaleRepositoryProvider).getTripSaleInvoiceById(data.items[index].id);

                                        mutate(future);
                                        try {
                                          LoadingOverlay.show();
                                          final tripSaleInvoice = await future;
                                          LoadingOverlay.hide();
                                          ref.read(productListNotifierProvider.notifier).setProductList(tripSaleInvoice.products);
